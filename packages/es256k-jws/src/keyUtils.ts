 ...
 * }
 * ```
 * See [rfc7517](https://tools.ietf.org/html/rfc7517) for more details on JWK.
 */

interface ISecp256k1JWK {
  /** See [rfc7517](https://tools.ietf.org/html/rfc7517) for more details on JWK. */
  kty: string; // 'EC';
  /** See [rfc7517](https://tools.ietf.org/html/rfc7517) for more details on JWK. */
  crv: string; // 'secp256k1';
  /** See [rfc7517](https://tools.ietf.org/html/rfc7517) for more details on JWK. */
  d?: string;
  /** See [rfc7517](https://tools.ietf.org/html/rfc7517) for more details on JWK. */
  x: string;
 ...
 */
export const getKid = (jwk: ISecp256k1JWK) => {
  const copy = { ...jwk };
  delete copy.d;
  delete copy.kid;
  delete copy.alg;
...
export const privateKeyHexFromJWK = async (jwk: ISecp256k1JWK) => {
  return keyto
    .from(
      {
        ...jwk,
...
export const publicKeyHexFromJWK = async (jwk: ISecp256k1JWK) => {
  const secp256k1 = await instantiateSecp256k1();
...
export const privateKeyUInt8ArrayFromJWK = async (jwk: ISecp256k1JWK) => {
  const privateKeyHex = await privateKeyHexFromJWK(jwk);
  return hexToBin(privateKeyHex);
};

/** convert jwk to binary encoded public key */
export const publicKeyUInt8ArrayFromJWK = async (jwk: ISecp256k1JWK) => {
